...
## webpack打包的结果，bundle 和 chunk 的区别？

## 为什么需要虚拟DOM？
1. 批量更新；
2. 框架设计；
3. 数据变化；数据变化 => 更新的最小颗粒度就是组件；在框架设计下，这种的设计，完成对真实DOM的更新。

## 说一下React.createElement的原理。
首先，react 在读取jsx 文件时，会对其中的html标签和组件进行一个类型判断， 如果是大写开头，那么就是组件，如果是小写开头，那么就是html标签。
createElement 方法有三个为一组的参数，每一组第一个是类型或者组件的方法，第二个是dom或者组件的属性。第三个则是它的children。返回的结果是虚拟DOM。